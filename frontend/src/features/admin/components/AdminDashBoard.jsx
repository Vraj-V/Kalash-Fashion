...
export const AdminDashBoard = () => {
    ...
    const totalItemsToday=orders
        .filter((order)=>isSameDay(order.createdAt))
        .reduce((sum,order)=>sum + (order.item?.reduce((acc,item)=>acc + (item.quantity || 0),0) || 0),0)
  ...
  return (
    ...
    <motion.div style={{position:"fixed",backgroundColor:"white",height:"100vh",padding:'1rem',overflowY:"scroll",width:is500?"100vw":"30rem",zIndex:500}}  variants={{show:{left:0},hide:{left:-500}}} initial={'hide'} transition={{ease:"easeInOut",duration:.7,type:"spring"}} animate={isProductFilterOpen===true?"show":"hide"}>
        ...
        <Stack rowGap={2} mt={4} >
            <Typography sx={{cursor:"pointer"}} variant='body2'>Lehenga</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Kurti</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Women Dress</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Saares</Typography>
            <Typography sx={{cursor:"pointer"}} variant='body2'>Choli</Typography>
        </Stack>
        ...
        <Grid container spacing={2} px={is600?2:4}>
            <Grid item xs={12} md={4} >
                <Paper elevation={2} sx={{p:3}} >
                    ...
                    <Typography variant='h5' color='#37c66e' fontWeight={600}>₹{totalSales}  </Typography>
                </Paper>
            </Grid>
            <Grid item xs={12} md={4}>
                <Paper elevation={2} sx={{p:3}}>
