...
exports.resendOtp=async(req,res)=>{
    try {
        ...
        console.log(`OTP for ${existingUser.email}: ${otp}`); // Temp - for testing

        await sendMail(
            existingUser.email,
            `OTP Verification for Your Kalash Fashion Account`,
            `Your One-Time Password (OTP) for account verification is: <b>${otp}</b>.</br>Do not share this OTP with anyone for security reasons`
        );

        res.status(201).json({'message':"OTP sent", otp, email:existingUser.email})
    } catch (error) {
...
exports.forgotPassword=async(req,res)=>{
    ...
    try {
        ...
        await sendMail(isExistingUser.email,'Password Reset Link for Your Kalash Fashion Account',`<p>Dear ${isExistingUser.name},

        We received a request to reset the password for your Kalash Fashion account. If you initiated this request, please use the following link to reset your password:</p>
